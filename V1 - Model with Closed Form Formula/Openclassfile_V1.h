...
public:
	...
	double q;  //risk factor: Probability of a susceptible student in the class to get infected
	double p;  //Probability of getting infected by a student j given they share a class together and social distance
...
string outputfilename2="DaySummaryMatrix.csv";  //outputfile2: Day*Summary Matrix. record # of students in different health conditions on each day
...
bool Openclassfile(const string& filename,int d) {
	...
	{   
		...
		while(comFile.good())
		{
			...
			if (s->InClass.size()!=0)   //Delete those students have no registered course
			{
        ...
        //Write first row of Day*Student Matrix, which are studentID of all students
		DayStudent.open(outputfilename1,std::ios::app);
		DayStudent<<"Day of the Semester"<<',';
		for(int i=0; i<StudentVector.size();i++)
		{
			if (i!=StudentVector.size()-1) {DayStudent<<StudentVector[i]<<',';}
			if (i==StudentVector.size()-1) {DayStudent<<StudentVector[i]<<'\r';}
		}
		DayStudent.close();
