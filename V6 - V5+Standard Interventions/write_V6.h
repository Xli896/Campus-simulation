...
bool WriteDaySummaryInv(int CumuQua, int CumuCase, int CumuNumSym,int DayNum,int NewExp,int k) {
	...
	{
		if(stu->ifQuarantine==1) {NumQua=NumQua+1;}
...
bool WriteDaySummaryIndividualHeader(int RunNum) {
	...
	DaySummaryInv<<"# Dead"<<',';
	DaySummaryInv<<"# Quarantine"<<',';
	DaySummaryInv<<"# Cumulative Quarantine"<<'\r';
	DaySummaryInv.close();
	return 0;
}
...
bool WriteAverageHeader() {
	...
	DaySummary<<"# Dead"<<',';
	DaySummary<<"# Quarantine"<<',';
	DaySummary<<"# Cumulative Quarantine"<<'\r';
	DaySummary.close();
	return 0;
}
...
bool WriteAverage() {
	...
	for (int i=0;i<=NumberofWeeks*DaysoftheWeek;i++)
	{
		...
		DaySummary<<DaySummaryMap[i][12]/RunTimes<<'\r';
	}
	DaySummary.close();
	return 0;
}
